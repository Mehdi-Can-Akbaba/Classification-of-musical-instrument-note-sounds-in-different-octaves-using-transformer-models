...
# Kaynak klasör (arp4lük)
source_dir = os.path.join(base_dir, 'piyano2lik')
...
if os.path.exists(source_dir):  # Kaynak klasör varsa
    ...
    for filename in os.listdir(source_dir):
        # Sadece "a4_" ile başlayan ve ".wav" uzantılı dosyaları seç
        if filename.startswith('p2_') and filename.endswith('.wav'):
            print(f'İşlenen dosya: {filename}')
            nota_name = filename[3:].split('.')[0]  # "a4_" kısmını çıkart
            target_folder = None
