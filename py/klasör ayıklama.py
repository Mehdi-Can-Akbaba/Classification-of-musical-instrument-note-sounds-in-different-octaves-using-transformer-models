...
# Kaynak klasör
source_dir = os.path.join(base_dir, 'elektro gitar', 'eg4lük')
...
# Nota türleri
orijinal_notalar = ['Do', 'Re', 'Mi', 'Fa', 'La', 'Si', 'Sol']
...
if os.path.exists(source_dir):  # Kaynak klasör varsa
    print(f'{source_dir} klasörü bulundu. Dosyalar kopyalanıyor...')
    for filename in os.listdir(source_dir):
        # Sadece "kg1_" ile başlayan ve ".wav" uzantılı dosyaları seç
        if filename.startswith('eg4_') and filename.endswith('.wav'):
            print(f'İşlenen dosya: {filename}')
            nota_name = filename[4:].split('.')[0]  # "kg1_" kısmını çıkart
            target_folder = None
